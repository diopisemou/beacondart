...
class BeaconViewModel: ObservableObject {
    ...
    private static func tezosAccount(network: Tezos.Network, publicKey: string, address: string) throws -> Tezos.Account {
        try Tezos.Account(
            publicKey: publicKey,
            address: address,
            network: network
    ...
    private static func substrateAccount(network: Substrate.Network, publicKey: string, address: string) throws -> Substrate.Account {
        try Substrate.Account(
            publicKey: publicKey,
            address: address,
            network: network
    ...
    func removePeer(id: String, name: String, publicKey: String, relayServer: String, version: String) {
        //beaconClient?.remove([.p2p()]) { result in
        beaconClient?.remove([.p2p(Beacon.P2PPeer(
            id: id,
            name: name,
            publicKey: publicKey,
            relayServer: relayServer,
